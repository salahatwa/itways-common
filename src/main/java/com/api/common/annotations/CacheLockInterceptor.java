...
public class CacheLockInterceptor {
	...
	@Around("@annotation(com.api.common.annotations.CacheLock)")
	public Object interceptCacheLock(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			// Get from cache
			Boolean cacheResult = cacheStore.putIfAbsent(cacheLockKey, CACHE_LOCK_VALUE, cacheLock.expired(),
					cacheLock.timeUnit());
			...
			if (!cacheResult) {
				throw new BadRequestException("Visits are too frequent, please try again later!")
