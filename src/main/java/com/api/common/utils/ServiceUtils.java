...
public class ServiceUtils {
	...
	public static <ID, T> Set<ID> fetchProperty(final Collection<T> datas, Function<T, ID> mappingFunction) {
		return CollectionUtils.isEmpty(datas) ? Collections.emptySet()
	...
	public static <ID, D> Map<ID, List<D>> convertToListMap(Collection<ID> ids, Collection<D> list,
		...
		list.forEach(
				data -> resultMap.computeIfAbsent(mappingFunction.apply(data), id -> new LinkedList<>()).add(data));
	 ...
	 */
	@NonNull
	public static <ID, D, V> Map<ID, V> convertToMap(@Nullable Collection<D> list, @NonNull Function<D, ID> keyFunction,
	...
	public static Pageable buildLatestPageable(int top) {
		return buildLatestPageable(top, "createdAt");
	}
...
//		return new PageImpl<>(Collections.emptyList(), PageRequest.of(page.getPage(), page.getSize()), page.getTotalElements());
		return null;
	}
