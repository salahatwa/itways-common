...
import static com.api.common.utils.JSON.stringify;
...
public class Page<T> {
	...
	public <U> Page<U> map(Function<? super T, ? extends U> converter) {
		List content = this.content.stream().map(converter).collect(Collectors.toList());
		return new Page<U>(content, this.page, this.size, this.totalElements);
	}

	public static <T> Page<T> empty() {
		return new Page<>(new ArrayList<>(), 0, 0, 0L);
	}
